...
> **Documento de produto (fonte de verdade) deste repositório.** Seguindo o `prd-good-pratices.md`,
> o PRD vive em `.claude/PRD.md` — este arquivo. Ele define estritamente **o quê** e **o porquê**;
> não descreve **como** o código é feito (isso é `docs/standards/`) nem regra de negócio de domínio
> (isso é `docs/domain/`).
>
> **Duplo papel (por ser um template):** aqui o conteúdo abaixo é o PRD do **próprio kit**. Quando
> alguém gera um projeto novo a partir deste template, o instalador **substitui** este arquivo por
> um esqueleto limpo (ver §7) — então, no projeto-alvo, `.claude/PRD.md` passa a ser a bússola
> **daquele** projeto, tratada como fonte de verdade pelos subagentes de `.claude/agents/`.
>
...
utilitários, um catálogo de plugins, uma trilha de subagentes `plan → run → test` e uma **estrutura
de pastas alinhada às boas práticas** (`prd-good-pratices.md`).
...
O repositório continua evoluindo de forma independente: novos agentes, skills, plugins, diretrizes
e melhorias de infraestrutura são adicionados aqui e passam a beneficiar todos os projetos futuros.
...
- **Caso de uso secundário:** evoluir o próprio template (adicionar agentes, skills, plugins,
  diretrizes, ajustar a infra) para que novos projetos herdem as melhorias.
...
### 3.1 Arquitetura de pastas e artefatos

Estrutura alinhada ao `prd-good-pratices.md` (itens específicos de domínio de dados, como
`data/lakehouse/`, ficam **fora** por serem amarrados a um tipo de projeto):

| Caminho / Arquivo                         | Propósito / função para o agente                                                                 |
| :---------------------------------------- | :----------------------------------------------------------------------------------------------- |
| `STATUS.md`                               | **Ponto de partida obrigatório.** Estado atual, o que foi feito e a próxima prioridade.          |
| `.claude/PRD.md`                          | **O "quê" e o "porquê".** Requisitos de produto, regras de alto nível e critérios de aceite.     |
| `.claude/plans/`                          | **O "como" da execução da IA.** Planos de `plan-dev`/`plan-ops` antes de executar.               |
| `.claude/agents/`                         | Os 5 subagentes do ciclo `plan → run → test`.                                                    |
| `.claude/skills/`                         | Ponto de extensão para skills reutilizáveis do Claude Code.                                       |
| `docs/domain/regras_negocio.md`           | **Fonte única da verdade do negócio.** Lógicas, cálculos e validações de domínio.                |
| `docs/standards/architecture.md`, `style.md` | **Engenharia e código.** Padrões de arquitetura, linting e estilo.                            |
| `docs/guidelines/`                        | **Comportamento da IA** (como criar subagentes, atualizar PRD/STATUS) + `prd-good-pratices.md` (referência ao escrever o PRD). |
| `.devcontainer/`                          | **Ambiente.** Regras de infra isoladas para agentes de ops; execução reprodutível.               |
| `scripts/`                                | **Ação.** Scripts executáveis (`clean.sh`, `plugins.sh`, e os que o projeto adicionar).          |
| `src/`                                    | **Código de produção.** Lógica central, isolada de ferramentas externas e orquestradores.        |
| `test/`                                   | **Controle de qualidade (TDD).** Testes que validam `src/` em loop.                              |

Princípios que essa estrutura materializa: **isolamento de escopo** (produto ≠ arquitetura ≠ regra
de negócio ≠ ambiente, cada um em seu arquivo), **single source of truth** (uma regra vive em um só
lugar) e **injeção de contexto sob demanda** (o agente lê o `STATUS.md` primeiro e, a partir dele,
só os arquivos que a tarefa exige).

### 3.2 Raiz
- **`install.sh`** / **`install.ps1`** — bootstrap (RF1–RF6, ver §4). Removidos do projeto gerado
  após o uso (só fazem sentido no template).
- **`STATUS.md`** — ponto de partida do trabalho no projeto (mantido no projeto gerado).
- **`README.md`** — uso do template (bootstrap + estrutura + fluxo manual alternativo).
- **`PRD.md` (não existe mais na raiz)** — o PRD do kit foi consolidado aqui em `.claude/PRD.md`.
...
- **`agents/`** — 5 subagentes **genéricos** (RF4) formando o ciclo `plan → run → test` em duas
  ...
  Sem amarração a um domínio; usam `.claude/PRD.md` do projeto gerado como fonte de verdade.
...
- **`docs/domain/`, `docs/standards/`** — esqueletos de regra de negócio e de padrões de
  arquitetura/estilo (placeholders a preencher).
- **`docs/guidelines/`** — diretrizes de comportamento da IA (README) e o `prd-good-pratices.md`
  (metodologia e arquitetura de pastas de referência, consultado ao escrever o `.claude/PRD.md`).
- **`src/`, `test/`** — pastas de código de produção e testes (só READMEs marcadores por enquanto).
...
- **`.claude/PRD.md`** — sobrescrito por um **esqueleto limpo** de PRD para o projeto-alvo (ver §7),
  no lugar do PRD do kit.
...
- Remove da raiz do projeto gerado o que só faz sentido no kit: `install.sh` e `install.ps1`.
- **Não** remove o restante (devcontainer, scripts, agentes genéricos, settings, `STATUS.md`,
  `docs/` — incluindo `docs/guidelines/prd-good-pratices.md`, referência para escrever o PRD —,
  `src/`, `test/`, `.claude/PRD.md` — este último é regenerado como esqueleto, não removido).
...
- O PRD do projeto-alvo vive em `.claude/PRD.md`. O instalador o **regenera como esqueleto**
  genérico (seções em branco + instruções) — ponto de partida do novo projeto, não conteúdo do kit.
...
- Operações destrutivas (remoção do `.git` do template, remoção do `install.*` da raiz) restritas
  ao clone temporário / ao diretório recém-criado do projeto.
...
- Estrutura específica de domínio de dados (`data/lakehouse/` com `bronze/silver/gold`) — o
  `prd-good-pratices.md` a descreve, mas ela não entra no template por ser amarrada a um tipo de
  projeto; fica como opção documentada para quem for construir um pipeline de dados.
...
## 7. Esqueleto de PRD do projeto-alvo

Quando o instalador gera um projeto novo, ele grava em `.claude/PRD.md` um esqueleto limpo (as
seções abaixo), no lugar deste PRD do kit. O desenvolvedor o preenche antes de acionar o ciclo
`plan → run → test`:

1. **Visão geral e propósito** — o quê o projeto é, o problema que resolve e o resultado esperado.
2. **Público-alvo e casos de uso** — quem usa e os principais cenários.
3. **Estado atual / contexto técnico** — stack, dependências, integrações e o que já existe.
4. **Requisitos funcionais** — uma seção por funcionalidade (RF1, RF2, …), com comportamento
   esperado e casos de borda relevantes o suficiente para virarem testes.
5. **Requisitos não-funcionais** — performance, segurança, portabilidade, integridade de dados.
6. **Fora de escopo** — o que o projeto explicitamente não vai fazer (por ora).
7. **Critérios de aceite** — checklist verificável do que precisa ser verdade para considerar
   pronto.

> O conteúdo literal desse esqueleto vive nos instaladores (`install.sh`/`install.ps1`), do mesmo
> jeito que o README mínimo do projeto-alvo — mantenha os três em sincronia ao evoluir o formato.

---

## 8. Ciclo de agentes e convenções de trabalho

Diretrizes de como trabalhar neste repositório (antes viviam no `CLAUDE.md`; consolidadas aqui).
Quando um projeto-alvo evoluir suas próprias diretrizes de comportamento da IA, o lugar natural é
`docs/guidelines/` — mantendo o **isolamento de escopo** (produto ≠ como a IA trabalha).

### 8.1 Idioma e uso do template

- Português é o idioma padrão de documentação e commits deste repositório.
- Ao trabalhar **dentro de um projeto gerado** a partir do template, a fonte de verdade do produto
  é o `.claude/PRD.md` local (regenerado como esqueleto pelo instalador) — preencha-o antes de
  acionar o ciclo abaixo. Comece sempre pelo `STATUS.md` (injeção de contexto sob demanda).

### 8.2 Ciclo `plan → run → test`
...
### 8.3 Convenções de código e ambiente
...
- Detalhes de arquitetura e estilo de código do projeto-alvo vivem em `docs/standards/`.
...
- Popular `docs/guidelines/` com diretrizes concretas de comportamento dos agentes.
- Ampliar o catálogo de `scripts/plugins.sh` (hoje: agent-browser, Context7, context-mode).
- Ensinar os agentes a começar pelo `STATUS.md` (injeção de contexto sob demanda) de forma explícita.
...
- [ ] `install.*` **não** estão presentes no projeto gerado; `.claude/PRD.md` permanece, mas
      **regenerado como esqueleto** (não com o conteúdo do kit).
- [x] Os agentes genéricos e demais arquivos do kit (devcontainer, scripts, settings, skills,
      `STATUS.md`, `docs/` — incluindo `docs/guidelines/prd-good-pratices.md` —, `src/`, `test/`)
      permanecem.
